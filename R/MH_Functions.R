...
FacetChromAO <- function(vcf, chromlist=NULL,windowSize=NULL,ncol=NULL){
  ...
  jpeg(file="plot9.jpeg")
  ggplot(data = SNPset, aes(x = AO)) + geom_point(color="purple") + facet_wrap(~CHROM, ncol = ncol) + theme_classic()
  dev.off()
  z<-  ggplot(data = SNPset, aes(x = AO)) + geom_point(color="purple") + facet_wrap(~CHROM, ncol = ncol) + theme_classic()
  print(z)
